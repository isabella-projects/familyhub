<x-layout>
    <div class="homepage--profile container py-md-5 container--narrow">
        <h2>
            ...
            <form class="ml-1 d-inline" action="/create-follow/{{$sharedData['username']}}" method="POST">
                @csrf
                <button class="btn btn-primary btn-sm follow">Follow <i class="fas fa-user-plus"></i></button>
            </form>
            @endif

            @if ($sharedData['isFollowing'])
            <form class="ml-1 d-inline" action="/remove-follow/{{$sharedData['username']}}" method="POST">
                @csrf
                <button class="btn btn-danger btn-sm follow">Stop Following <i class="fas fa-user-times"></i></button>
            </form>
            @endif

            @if (auth()->user()->username == $sharedData['username'])
            <a href="/manage-avatar" class="btn btn-secondary btn-sm avatar">Manage Avatar</a>
            @endif
